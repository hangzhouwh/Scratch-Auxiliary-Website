    ...
    <el-table
      ...
      <el-table-column width="60"
        ...
        <template slot-scope="scope">
          <el-tooltip effect="light" content="Lyrics match score" placement="top">
            <span>{{toFixed(scope.row.score)}}</span>
          </el-tooltip>
        </template>
      </el-table-column>
...
<script>
  export default {
    ...
    watch: {
      listSearchInput (val) {
        this.getSearchResultLyric()
      }
    },
    methods: {
      getSearchResultLyric (page_num) {
        console.log('读取全局变量search_input:' + this.$store.state.search_input
          + '读取全局变量host:' + this.$store.state.user_info.host)
        this.SearchResultForm.lyric = this.$store.state.search_input
      ...
      deleteFavouriteSong (song_id) {
        ...
        this.$axios.post(`collection/${this.host}/delete_song?song_id=${song_id}`).then(response => {
          ...
          this.getSearchResultLyric(this.data.page_num)
        }).catch((error) => {
          console.log(error)
        })
      },
